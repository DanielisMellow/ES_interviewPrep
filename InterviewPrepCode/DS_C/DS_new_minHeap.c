...
min_heap* create_heap() {
    ...
    if((new_heap->nodes = (node **)malloc(sizeof(node) * new_heap->cap)) == NULL) {
        return NULL; 
    }
    return new_heap; 
}
...
void insert_to_heap(min_heap *min_heap, char key, int value) {
    ...
    while(i && min_heap->nodes[i]->value < min_heap->nodes[parent_index(i)]->value) {
...
void heapify(min_heap *min_heap, int index) {
    ...
    if(left < min_heap->size && min_heap->nodes[left]->value < min_heap->nodes[smallest]->value) {
        smallest = left; 
    }
    if(right < min_heap->size && min_heap->nodes[right]->value < min_heap->nodes[smallest]->value) {
        smallest = right; 
    }
    if(smallest != index) {
...
int main(void) {
    ...
    printf("\nNow in the heap...\n"); 
    for(int i = 0; i < 7; i++) {
       printf("Key: %c, Value: %d\n", my_heap->nodes[i]->key, my_heap->nodes[i]->value); 
    }
