...
void swap(void *a, void *b, int size) {
    void *tmp; 
    if((tmp = malloc(sizeof(size))) == NULL) { return; }
    memcpy(tmp, a, size); 
    memcpy(a, b, size); 
    memcpy(b, tmp, size); 
    free(tmp); 
...
heap_node * create_new_node(unsigned int vertex, unsigned int weight) {
    heap_node *new_node; 
    if((new_node = (heap_node*)malloc(sizeof(heap_node))) == NULL) { return NULL; }
...
min_heap * create_new_heap(int capacity) {
    min_heap *new_heap; 
    if((new_heap = (min_heap*)malloc(sizeof(min_heap))) == NULL) { return NULL; }
    new_heap->cap = capacity; 
    new_heap->size = 0; 
    if((new_heap->pos = (int*)malloc(sizeof(int) * new_heap->cap)) == NULL) { return NULL; }
    if((new_heap->nodes = (heap_node**)malloc(sizeof(heap_node) * new_heap->cap)) == NULL) { return NULL; }
    return new_heap; 
}
...
void insert_into_heap(min_heap *min_heap, unsigned int vertex, unsigned int weight) {
    ...
    while(index && min_heap->nodes[index]->weight < min_heap->nodes[parent_index(index)]->weight) {
        ...
        swap(min_heap->nodes[start], min_heap->nodes[parent_index(index)], sizeof(heap_node)); 
...
void heapify(min_heap *min_heap, int index) {
    ...
    if(left < min_heap->size && min_heap->nodes[left]->weight < min_heap->nodes[smallest]->weight) {
        smallest = left; 
    }
    if(right < min_heap->size && min_heap->nodes[right]->weight < min_heap->nodes[smallest]->weight) {
        smallest = right; 
    }
    if(smallest != index) {
...
int extract_min(min_heap *min_heap) {
    if(is_heap_empty(min_heap)) { return INT32_MIN; }
    ...
    swap(min_heap->nodes[0], min_heap->nodes[min_heap->size - 1], sizeof(heap_node)); 
    min_heap->size--; 
...
void decrease_key(min_heap *min_heap, unsigned int vertex, unsigned int weight) {
    ...
    while(i && min_heap->nodes[i]->weight < min_heap->nodes[parent_index(i)]->weight) {
        ...
        swap(min_heap->nodes[i], min_heap->nodes[parent_index(i)], sizeof(heap_node)); 
        i = parent_index(i); 
    }
}
...
//     printf("\nNow in the heap...\n"); 
//     for(int i = 0; i < 7; i++) {
//        printf("Key: %c, Value: %d\n", my_heap->nodes[i]->vertex, my_heap->nodes[i]->weight); 
//     }
//
//     printf("\n"); 
//     printf("pos array:\n"); 
