...
void print_set(int parents[], int distance[], int starting_vertex, graph *graph) {
    if(!(graph) || !(parents)) { return; }

    printf("Vetex\tDistance\tPath\n"); 

    for(int i = 0; i < graph->vertex_amount; i++) {
        printf("%d -> %d\t%d\t\t%d ", starting_vertex, i, distance[i], starting_vertex); 
...
void dijkstra_alg(graph *graph, int starting_vertex) {
    ...
    while(!(is_heap_empty(min_heap))) {
        ...
        for(int v = 0; v < graph->vertex_amount; v++) {
            ...
            if(u_v->weight && is_in_heap(min_heap, v) && distance[u] != INT32_MAX && tmp_distance < distance[v]) {
